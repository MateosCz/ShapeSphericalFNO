...
if __name__ == "__main__":
    ...
    grid_num = 15
    grid_range = [-1,1]
    sphere_data_generator_XT = S2ManifoldDataGenerator(sampling="gl", manifold_type='real_data', file_path=project_root() + "/data/morph/Lepus_capensis.stl", scale=0.5, src_type='mesh')
    # sphere_data_generator_XT = S2ManifoldDataGenerator(sampling="gl", manifold_type='real_data', file_path=project_root() + "/data/morph/Lepus_saxatilis.stl", scale=0.5, src_type='mesh')
    sphere_data_generator_X0 = S2ManifoldDataGenerator(sampling="gl", manifold_type='real_data', file_path=project_root() + "/data/morph/Lepus_californicus.stl", scale=0.5, src_type='mesh')
    ...
    if not draw_unconditional:
        ...
        retrain_checkpoint_path = project_root() + '/checkpoints/lepus_model_retrain'
    ...
    def normalize_mesh(mesh, rescale_factor=1.0):
        ...
        return mesh
    mesh_path = project_root() + "/data/morph/Lepus_californicus.stl"
    x0_mesh = o3d.io.read_triangle_mesh(mesh_path)
    x0_mesh = normalize_mesh(x0_mesh, 0.5)
    save_path = project_root() + "/figures/thesis/"

    ps_3d_time_slice(condition_xs, [0,0.3,0.6,1.0], x_T_mesh=x0_mesh, show_traj=False, save_path=save_path, store_title="lepus_experiment_xT_lepus_capensis")