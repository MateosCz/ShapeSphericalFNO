...
if __name__ == "__main__":
    ...
    print(x0.shape)
    sde_3d = Kunita_Flow_SDE_3D_Eulerian_2Dmanifold_distance(k_alpha=0.8, k_sigma=0.4, grid_num=12, grid_range=[-2,2], x0=x0[0])
    ...
    if not draw_unconditional:
        model = CTShapeSFNO(x_feature_dim=3, l_list=(16,8,4), lift_dim=16, latent_feature_dims=(1,2,4), sampling="gl", activation="gelu")
        trainer = Trainer.NeuralOpTrainer(seed=get_random_int(), landmark_num=in_grid_L)

        checkpoint_path = project_root() + '/checkpoints/real_data_model'
        retrain_checkpoint_path = project_root() + '/checkpoints/real_data_model_retrain'
