...
if __name__ == "__main__":
    ...
    in_grid_L = 12
    sphere_data_generator_XT = S2ManifoldDataGenerator(radius=0.7, sampling="gl", manifold_type="cylinder", seed=get_random_int(), randomization=False)

    xT = sphere_data_generator_XT.generate_data(in_grid_L, 1)
    print(xT.shape)
    sphere_data_generator_X0 = S2ManifoldDataGenerator(radius=0.5, sampling="gl", manifold_type="cylinder", seed=get_random_int(), randomization=False)
    ...
    if not draw_unconditional:
        ...
        checkpoint_path = project_root() + '/checkpoints/cylinder_model'
        retrain_checkpoint_path = project_root() + '/checkpoints/cylinder_model_retrain'
